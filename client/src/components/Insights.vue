  ...
  <a-spin :spinning="loadingSurveys">
    <a-space direction="vertical" :size="24" style="width: 100%;">
      <a-card 
        ...
        :loading="insightsState[question.id]?.loading"
      >
...
import AnimationWordcloud from './AnimationWordcloud.vue'
...
async function loadSurveys() {
  ...
  try {
    // 获取所有选中的问卷
    const surveyPromises = props.surveyIds.map(id => apiService.getSurvey(id))
...
async function loadInsight(questionId: number) {
  if (insightsState.value[questionId]) {
    return
  }

  insightsState.value[questionId] = { loading: true }
  try {
    const insight = await apiService.getCrossInsight(questionId, props.surveyIds)
    insightsState.value[questionId] = { response: insight, loading: false }
  } catch (error) {
...
function getStarDistribution(questionId: number): Record<number, number> {
  const state = insightsState.value[questionId]
  if (!state?.response || props.surveyIds.length !== 1) return {}
...
function getStarAverage(questionId: number): number {
  const state = insightsState.value[questionId]
  if (!state?.response || props.surveyIds.length !== 1) return 0
